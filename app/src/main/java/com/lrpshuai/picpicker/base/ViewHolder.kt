...
class ViewHolder(private val mContext: Context, parent: ViewGroup, val layoutId: Int,
    ...
    fun setImageUrl(viewID: Int, url: String): ViewHolder {
        ...
        if (!TextUtils.isEmpty(url)) {
            Glide.with(mContext).load(url)
                    .asBitmap()
                    .placeholder(R.mipmap.icon_user_defult_avatar)
                    .error(R.mipmap.icon_user_defult_avatar)
                    .dontAnimate()
                    .into(view)
        }
        return this
    ...
    fun setImageUrl(viewID: Int, url: String, resource: Int): ViewHolder {
        val view = getView<ImageView>(viewID)
        if (!TextUtils.isEmpty(url)) {
            Glide.with(mContext).load(url)
                    .asBitmap()
    ...
    fun setAlpha(viewId: Int, value: Float): ViewHolder {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            val alpha = AlphaAnimation(value, value).apply {
                duration = 0
                fillAfter = true
            }
    ...
    fun setHolderVisible(viewId: Int, visible: Boolean): ViewHolder {
        val view = getView<View>(viewId)
