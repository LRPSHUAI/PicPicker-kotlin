...
public class CompressPic {
    ...
    private static volatile CompressPic INSTANCE;
    private OnCompressListener compressListener;
    private File mFile;
    private int gear = THIRD_GEAR;
    private String filename;

    private CompressPic(File cacheDir) {
        File mCacheDir = cacheDir;
    }

    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, CompressPic.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    private static File getPhotoCacheDir(Context context, String cacheName) {
        ...
        if (cacheDir != null) {
            ...
            File comp = new File(cacheDir + "/.comp");
            if (!comp.mkdirs() && (!comp.exists() || !comp.isDirectory())) {
                return null;
            }
    ...
    public static CompressPic get(Context context) {
        if (INSTANCE == null) INSTANCE = new CompressPic(CompressPic.getPhotoCacheDir(context));
        return INSTANCE;
    }

    public CompressPic launch() {
        if (compressListener != null) compressListener.onStart();

        if (gear == CompressPic.FIRST_GEAR)
        ...
        else if (gear == CompressPic.THIRD_GEAR)
            Observable.just(mFile)
                    .map(new Func1<File, File>() {
                        @Override
                        public File call(File file) {
    ...
    public CompressPic load(File file) {
        mFile = file;
        return this;
    }

    public CompressPic setCompressListener(OnCompressListener listener) {
        compressListener = listener;
        return this;
    }

    public CompressPic putGear(int gear) {
        this.gear = gear;
        return this;
    }

    public CompressPic setFilename(String filename) {
    ...
    private File firstCompress(@NonNull File file) {
        ...
        return compress(filePath, thumb, width, height, angle, size);
    }
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) {
        ...
        while (stream.toByteArray().length / 1024 > 500 && options > 50) {
            stream.reset();
            options -= 6;
            bitmap.compress(Bitmap.CompressFormat.JPEG, options, stream);
        }
